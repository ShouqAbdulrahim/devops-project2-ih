...
# Data lookups (existing VNet/Subnet)
...
# App Insights (create new or skip if you pass connection string)
...
resource "azurerm_linux_web_app" "backend" {
  ...
  site_config {
    ...
    ftps_state = "Disabled"
    application_stack {
      java_version = format("%d", var.java_major_version) # e.g. 17
      java_server  = "JAVA"
    }
...
# VNet Integration for both apps (Subnet must be delegated to Microsoft.Web/serverFarms)
...
# Access Restrictions → allow ONLY App Gateway public IP
