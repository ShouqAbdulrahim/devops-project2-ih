...
resource "azurerm_network_security_group" "nsg_frontend" {
  name                = "nsg-frontend-shouq"
  location            = azurerm_resource_group.rg.location
  resource_group_name = azurerm_resource_group.rg.name
}

resource "azurerm_network_security_group" "nsg_backend" {
  name                = "nsg-backend-shouq"
  location            = azurerm_resource_group.rg.location
  resource_group_name = azurerm_resource_group.rg.name
}
...
# قواعد رئيسية (سنضبط المنافذ المطلوبة لاحقًا مع AppGW و SQL)
# السماح من Subnet AppGW إلى Frontend (80/443)
...
# السماح من Subnet AppGW إلى Backend (8080)
...
# السماح من Subnet mgmt إلى SSH (22) داخل Frontend/Backend (لتشغيل Ansible داخليًا)
...
# السماح من Backend إلى Data (1433) — للوصول إلى Private Endpoint الخاص بـ Azure SQL
...
# ربط الـ NSGs مع الشبكات الفرعية
